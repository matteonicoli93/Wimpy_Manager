...
def readNOAAwebsite(link):
    ...
    NOAA_Pass = DataFrame (data, columns = ['Date','S/C','Text'])
    return NOAA_Pass
...
def NOAA_week_sch():
    ...
    NOAA_week = NOAA_week.drop_duplicates(subset=['Date', 'S/C', 'Text'])
    NOAA_week = NOAA_week.reset_index()
    NOAA_week = NOAA_week.drop(columns='index')
...
def NOAA_BOS_shift(FDF_Pass_N19, DS, DE, filePath_NOAA_week_OLD):
    ...
    if (path.isfile(file_Path_Name_NOAA_week_OLD) == True):
        NOAA_week_OLD = read_csv(file_Path_Name_NOAA_week_OLD)
    ...
    for wrow in range(0,NOAA_week.shape[0]):
        ...
        if ( iDateText[:len('RECEIVE,H')]=='RECEIVE,H' and iDate >= DS and iDate <= DE ):
            flag = 1
            idata = []
        ...
        if ( flag == 1):
            if (iDateText[:len('RECEIVE,G')]=='RECEIVE,G' ):
                if (iDateText[15:18]=='MSB'):
                    idata.append(iDateText[12:14])
                    idata.append(str(int(iDateText[31:36])*2))
           
            if (iDateText[:1]=='A'):
                idata.append(iDate)
            
            if (iDateText[:len('PBK,S')]=='PBK,S' ):
                if (iDateText[-len('MSB'):]=='MSB'):
                    idata.append(iDate)
            
            if (iDateText[:len('PBK,E')]=='PBK,E' ):
                if (iDateText[-len('MSB'):]=='MSB'):
                    idata.append(iDate)
            
            if ( iDateText[-len('END'):]=='END'):
                idata.append(iDate)
    ...
    if (npass!=0):
        while (flag == 1):
            ...
            if (nOrbit_FDF == nOrbit_BOS):
                FDF_Pass_N19.at[wrow,'CDA_AOS']=N_NOAA_Pass_shift[iBOS][4]
                FDF_Pass_N19.at[wrow,'CDA_LOS']=N_NOAA_Pass_shift[iBOS][-1]
                GAC_Type = N_NOAA_Pass_shift[iBOS][2]
                #print (nOrbit_BOS, (NOAA_Pass_shift.loc[iBOS*10+2,['Text']]['Text']))
                GAC_Frame = int(N_NOAA_Pass_shift[iBOS][3])
                FDF_Pass_N19.at[wrow,'GDS_GAC']=GAC_Type+','+str(GAC_Frame)
                FDF_Pass_N19.at[wrow,'NOAA_BOS']='YES'
                FDF_Pass_N19.at[wrow,'Dump_Start']=N_NOAA_Pass_shift[iBOS][5]
                FDF_Pass_N19.at[wrow,'Dump_End']=N_NOAA_Pass_shift[iBOS][6]
                iBOS = iBOS + 1
                if (iBOS == npass):
                    flag = 0
            wrow = wrow + 1
